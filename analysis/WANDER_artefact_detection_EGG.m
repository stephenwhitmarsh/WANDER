function [artdef] = WANDER_artefact_detection_EGG(isubject,force,rootpath,restingstate)
...
%         EGG_bb.trial{1} = zscore(EGG.trial{1},0,2);
        ...
        for ipart = 1:4
            ...
            %smooth data
            EGG{ipart}.trial{1}(3,:) = smooth(EGG{ipart}.trial{1}(3,:),5);
            
            %differentials
            EGG{ipart}.trial{1}(5,:) = [0 diff(EGG{ipart}.trial{1}(3,:),1,2)];
            EGG{ipart}.trial{1}(6,:) = [0 0 diff(EGG{ipart}.trial{1}(3,:),2,2)];
            artperiod         = artedge:size(EGG{ipart}.trial{1},2)-artedge;
            
            %ztransform without edges
            EGG{ipart}.trial{1}(5,:) = EGG{ipart}.trial{1}(5,:) - mean(EGG{ipart}.trial{1}(5,artperiod));
            EGG{ipart}.trial{1}(5,:) = EGG{ipart}.trial{1}(5,:) ./ std(EGG{ipart}.trial{1}(5,artperiod));
            EGG{ipart}.trial{1}(6,:) = EGG{ipart}.trial{1}(6,:) - mean(EGG{ipart}.trial{1}(6,artperiod));
            EGG{ipart}.trial{1}(6,:) = EGG{ipart}.trial{1}(6,:) ./ std(EGG{ipart}.trial{1}(6,artperiod));
            
            %smooth data
            EGG{ipart}.trial{1}(5,:) = smooth(EGG{ipart}.trial{1}(5,:),5);
            EGG{ipart}.trial{1}(6,:) = smooth(EGG{ipart}.trial{1}(6,:),5);
            
            %define artifacts
            artval            = zeros(1,size(EGG{ipart}.trial{1},2));
            artval(artperiod) = EGG{ipart}.trial{1}(6,artperiod) > threshold;
            ...
            fig = figure('units','normalized','outerposition',[0 0 1 1]);
            subplot(6,1,1);
            plot(EGG{ipart}.trial{1}(1,:));    axis tight; title([num2str(isubject) ' orig']);
            ax = axis;
            axis([ax(1),ax(2),-5,5]);
            line([ax(1) ax(2)],[4 4],'color','r');
            line([ax(1) ax(2)],[-4 -4],'color','r');
            ...
            subplot(6,1,2);
            plot(EGG{ipart}.trial{1}(2,:));    axis tight; title([num2str(isubject) ' phase']);
            ax = axis;
            axis([ax(1),ax(2),-5,5]);
            line([ax(1) ax(2)],[4 4],'color','r');
            line([ax(1) ax(2)],[-4 -4],'color','r');
            ...
            subplot(6,1,3);
            plot(EGG{ipart}.trial{1}(3,:));    axis tight; title([num2str(isubject) ' amplitude']);
            ax = axis;
            axis([ax(1),ax(2),-5,5]);
            line([ax(1) ax(2)],[4 4],'color','r');
            line([ax(1) ax(2)],[-4 -4],'color','r');
            ...
            subplot(6,1,4);
            plot(EGG{ipart}.trial{1}(5,:));    axis tight; title([num2str(isubject) ' diff(amplitude)']);
            ax = axis;
            axis([ax(1),ax(2),-5,5]);
            line([ax(1) ax(2)],[4 4],'color','r');
            line([ax(1) ax(2)],[-4 -4],'color','r');
            ...
            subplot(6,1,5);
            plot(EGG{ipart}.trial{1}(6,:));    axis tight; title([num2str(isubject) ' diff2(amplitude)']);
            ax = axis;
            axis([ax(1),ax(2),-threshold*1.5,threshold*1.5]);
            line([ax(1) ax(2)],[threshold threshold],'color','r','linestyle',':');
            line([ax(1) ax(2)],[-threshold -threshold],'color','r','linestyle',':');
            ...
            subplot(6,1,6);
            plot(EGG{ipart}.trial{1}(4,:));    axis tight; title([num2str(isubject) ' narrow filter']);
            ax = axis;
            axis([ax(1),ax(2),-5,5]);
            line([ax(1) ax(2)],[4 4],'color','r');
            line([ax(1) ax(2)],[-4 -4],'color','r');

            artdef{ipart} = artifact_ext;
            
            print(fig,'-painters','-dpng',['d:\analysis\WANDER\images\EGG\overview_s' num2str(isubject) '_block_' num2str(ipart) '.png']);
        end
    end
