...
timing = 'cue'; % all data is saved in "cue", also probe-locked
for isubject = slist
    fprintf('Loading subject %d \n',isubject);    
    temp = load(['/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/data/ERF/s' num2str(isubject) '_' timing '.mat'],'ERF_probe','ERF_probe_high','ERF_probe_low');
    ...
    ERF{isubject}       = rmfield(ERF{isubject}, 'cfg');
    ERF_low{isubject}   = rmfield(ERF_low{isubject}, 'cfg');
    ERF_high{isubject}  = rmfield(ERF_high{isubject}, 'cfg');    
end

save('/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/data/ERF/GA_revision.mat','ERF','ERF_low','ERF_high', '-v7.3');
% load('/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/data/ERF/GA_revision.mat','ERF','ERF_low','ERF_high');
load('/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/scripts/grad.mat');

%% Calculate FFT at SSSF frequency
for isubject = slist
    cfg                         = [];
    cfg.pad                     = 'nextpow2';
    cfg.channel                 = 'all';
...
%     cfg.toi                     = -9.5:0.05:0.45;
    ...
    FFT{isubject}               = ft_freqanalysis(cfg, ERF{isubject});
    FFT_high{isubject}          = ft_freqanalysis(cfg, ERF_high{isubject});
    FFT_low{isubject}            = ft_freqanalysis(cfg, ERF_low{isubject});
    
    FFT{isubject}.grad          = grad;
    FFT_high{isubject}.grad     = grad;
    FFT_low{isubject}.grad      = grad; 
end
save('/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/data/ERF/GA_TFR_revision.mat','FFT','FFT_high','FFT_low', '-v7.3');

% combine planar
for isubject = slist
    FFT{isubject}      = ft_combineplanar([], FFT{isubject});
    FFT_high{isubject} = ft_combineplanar([], FFT_high{isubject});
    FFT_low{isubject}  = ft_combineplanar([], FFT_low{isubject});
end

% average over subjects
cfg                     = [];
cfg.keepindividual      = 'yes';
FFT_GA                  = ft_freqgrandaverage(cfg, FFT{slist});
FFT_high_GA             = ft_freqgrandaverage(cfg, FFT_high{slist});
FFT_low_GA              = ft_freqgrandaverage(cfg, FFT_low{slist});
...
temp                    = ft_selectdata(cfg,FFT_GA);
[Y I]                   = sort(temp.powspctrm,'ascend');
chan_FFT_mag            = ft_channelselection(I(end), temp.label);

cfg.channel             = 'MEG*3';
temp                    = ft_selectdata(cfg,FFT_GA);
[Y I]                   = sort(temp.powspctrm,'ascend');
chan_FFT_cmb            = ft_channelselection(I(end), temp.label);

% select the channel
cfg = [];
cfg.channel             = chan_FFT_mag;
FFT_GA_sel              = ft_freqgrandaverage(cfg, FFT_GA);
FFT_high_GA_sel         = ft_freqgrandaverage(cfg, FFT_high_GA);
FFT_low_GA_sel          = ft_freqgrandaverage(cfg, FFT_low_GA);

high = permute(FFT_high_GA_sel.powspctrm, [1, 3, 2]);
low  = permute(FFT_low_GA_sel.powspctrm, [1, 3, 2]);

figure; hold;
subplot(3,1,1); hold;
errorbar(FFT_high_GA_sel.freq, mean(low),  std(low)  ./ sqrt(22));
errorbar(FFT_high_GA_sel.freq, mean(high), std(high) ./ sqrt(22));
xlim([14, 18]);

[h, p] = ttest(low, high);
subplot(3,1,2);
plot(FFT_high_GA_sel.freq, p)
xlim([14, 18]);

subplot(3,1,3);
plot(FFT_high_GA_sel.freq, h)
xlim([14, 18]);

summary(1, :) = FFT_high_GA_sel.freq;
summary(2, :) = p;
summary(3, :) = h;
...
for isubject = slist
    ...
    TFR{isubject}.grad          = grad;
    TFR_high{isubject}.grad     = grad;
    TFR_low{isubject}.grad      = grad; 
    TFR{isubject}               = rmfield(TFR{isubject},        'cfg');
    TFR_high{isubject}          = rmfield(TFR_high{isubject},   'cfg');
    TFR_low{isubject}           = rmfield(TFR_low{isubject},    'cfg');
end
save('/network/lustre/iss01/charpier/analyses/stephen.whitmarsh/WANDER/data/ERF/GA_TFR_revision.mat','TFR','TFR_high','TFR_low', '-v7.3');
...
% get maximum sensor from individuals

for isubject = slist
 
    % get maximum sensor from GA
    ...
    [~, I]                  = sort(temp.powspctrm,'ascend');
    
    cfg                                     = [];
    cfg.channel                             = ft_channelselection(I(end), temp.label);
    TFR_ind_max_mag{isubject}               = ft_selectdata(cfg,TFR{isubject});
    TFR_ind_max_mag{isubject}.label{1}      = 'MAG';
    TFR_ind_max_mag_high{isubject}          = ft_selectdata(cfg,TFR_high{isubject});
    TFR_ind_max_mag_high{isubject}.label{1} = 'MAG';
    TFR_ind_max_mag_low{isubject}           = ft_selectdata(cfg,TFR_low{isubject});
    TFR_ind_max_mag_low{isubject}.label{1}  = 'MAG';
    ...
    cfg                                         = [];
    cfg.channel                                 = ft_channelselection(I(end), temp.label);
    TFR_ind_max_grad{isubject}                  = ft_selectdata(cfg,TFR{isubject});
    TFR_ind_max_grad{isubject}.label{1}         = 'GRAD';
    TFR_ind_max_grad_high{isubject}             = ft_selectdata(cfg,TFR_high{isubject});
    TFR_ind_max_grad_high{isubject}.label{1}    = 'GRAD';
    TFR_ind_max_grad_low{isubject}              = ft_selectdata(cfg,TFR_low{isubject});
    TFR_ind_max_grad_low{isubject}.label{1}     = 'GRAD';
...
cfg.channel             = chan_TFR_mag;
stat_TFR_mag            = ft_freqstatistics(cfg, TFR_low{slist}, TFR_high{slist});

a(1,:) = stat_TFR_mag.freq;
a(2,:) = stat_TFR_mag.prob;

cfg.channel             = chan_TFR_cmb;
stat_TFR_cmb            = ft_freqstatistics(cfg, TFR_low{slist}, TFR_high{slist});

b(1,:) = stat_TFR_cmb.freq;
b(2,:) = stat_TFR_cmb.prob;

cfg.channel         = 'MAG';
stat_TFR_max_mag    = ft_freqstatistics(cfg, TFR_ind_max_mag_low{slist},  TFR_ind_max_mag_high{slist});

c(1,:) = stat_TFR_max_mag.freq;
c(2,:) = stat_TFR_max_mag.prob;

cfg.channel         = 'GRAD';
stat_TFR_max_grad   = ft_freqstatistics(cfg, TFR_ind_max_grad_low{slist}, TFR_ind_max_grad_high{slist});

d(1,:) = stat_TFR_max_grad.freq;
d(2,:) = stat_TFR_max_grad.prob;


figure; 

subplot(4,1,1); hold;
plot(stat_TFR_mag.freq, stat_TFR_mag.prob);
plot([1, 30], [0.05, 0.05]);

subplot(4,1,2); hold;
plot(stat_TFR_cmb.freq, stat_TFR_cmb.prob)
plot([1, 30], [0.05, 0.05]);

subplot(4,1,3); hold;
plot(stat_TFR_max_mag.freq, stat_TFR_max_mag.prob)
plot([1, 30], [0.05, 0.05]);

subplot(4,1,4); hold;
plot(stat_TFR_max_grad.freq, stat_TFR_max_grad.prob)
plot([1, 30], [0.05, 0.05]);
...
low = squeeze(TFR_low_sel_avg.powspctrm);
high = squeeze(TFR_high_sel_avg.powspctrm);


figure;
subplot(3,1,1); hold;
errorbar(TFR_low_sel_avg.freq, mean(low),std(low)   ./ sqrt(22));
errorbar(TFR_low_sel_avg.freq, mean(high),std(high) ./ sqrt(22));

[h, p] = ttest(low, high);

subplot(3,1,2);
plot(TFR_low_sel_avg.freq, p)

subplot(3,1,3);
plot(TFR_low_sel_avg.freq, h)

summary(1, :) = TFR_low_sel_avg.freq;
summary(2, :) = p;
summary(3, :) = h;


t_freqgrandaverage(cfg,TFR_diff_sel{slist});
