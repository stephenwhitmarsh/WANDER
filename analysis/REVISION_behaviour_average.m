function REVISION_behaviour_average
...
for isubject = slist
    behav                                                       = REVISION_behaviour(isubject,0,0);
...
%% TABLE FOR REVISION
behaviour = table;
behaviour.correct_rejection = round(rejectionrate(slist), 3)*100;
behaviour.hitrate = round(hitrate(slist), 3)*100;
behaviour.missrate = round(missrate(slist), 3)*100;
behaviour.FArate = round(FArate(slist), 3)*100;
