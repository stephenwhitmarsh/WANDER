...
for isubject = slist
    ...
    fname_SSEF_phase = ['D:\WANDER\data\SSEF\SSEF_phase_' num2str(isubject) '.mat'];
...
% patch([time,time(end:-1:1)], [mean(max_timecourse_sel(slist,:)) + std(max_timecourse_sel(slist,:)), mean(max_timecourse_sel(slist,end:-1:1)) - std(max_timecourse_sel(slist,end:-1:1))],[0 0 0],'LineStyle','None','FaceAlpha',0.5);
% plot(mean(max_timecourse_sel(slist,:)),'k'); 
...
axis tight
% 
% plot(mean(max_timecourse_sel(slist,:)) + std(max_timecourse_sel(slist,:)),'k:'); axis tight;
% plot(mean(max_timecourse_sel(slist,:)) - std(max_timecourse_sel(slist,:)),'k:'); axis tight;
...
subplot(3,1,3);
% plot(mean(min_timecourse_sel(slist,:))); axis tight;
...
subplot(3,1,1);
hold;
plot(mean(max_timecourse_sel(slist,:)),'k'); axis tight;
plot(mean(max_timecourse_sel_high(slist,:)),'r'); axis tight;
plot(mean(max_timecourse_sel_low(slist,:)),'b'); axis tight;
title('mean timecourse');
xlabel('ms');

subplot(3,1,2);
plot(mean(max_timecourse_sel_high(slist,:) / max_timecourse_sel_low(slist,:)),'r'); axis tight;
...
subplot(3,1,3);
[h,p,ci,stats] = ttest(max_timecourse_sel_high,max_timecourse_sel_low);
plot(stats.tstat);
...
% figure; hold;
% for isubject = slist
%     q(isubject,:) = (ori_template{isubject}' * pos_template{isubject}')';
%     plot(q(isubject,:));
% end
% figure; 
% plot(mean(q(slist,:)))
...
% 
% figure; plot(ori_template{isubject}' * pos_template{isubject}')
% 
% pos_template{isubject} 
...
% tabdelimited
dlmwrite('D:\analysis\WANDER\dipoles3.node',d,'\t')
...
%         corr_trial{isubject}(itrial,:) = corr(squeeze(source_TFR{isubject}.powspctrm(itrial,1,16,:)),squeeze(source_TFR{isubject}.powspctrm(itrial,1,:,:))','rows','complete');   
%     end   
%         
...
%     corr_trial_avg_high(isubject,:) = nanmean(corr_trial{isubject}(trialinfo{isubject}(:,6) == 2));
%     corr_trial_avg_low(isubject,:)  = nanmean(corr_trial{isubject}(trialinfo{isubject}(:,6) == 1));
% 
% end
% 
% figure;
% imagesc(corr_trial_avg(slist,:)')
% plot(mean(corr_trial_avg(slist,:)));
% plot(corr_trial{isubject}')
% 
% isubject = 1;
% t = squeeze(source_TFR{isubject}.powspctrm(:,1,10,:));
% t(isnan(t)) = 10
% figure; imagesc(t)
% 
% figure; imagesc(squeeze(source_TFR{isubject}.powspctrm(:,1,10,:)))
% figure; 
% 
% ft_databrowser([],source{isubject});
% )
% 
% c = colormap
% c(1,:) = [1 1 1];
...
%     figure; imagesc(squeeze(source_TFR.powspctrm(itrial,1,:,:)));
%     nanmean(corr_trial);
%     
%     
...
% save('dipole_sources','source_cue*','source_probe*','-v7.3'); 
% 
...
% xticklabels = latency_cue(1:25:end)/1000-1;
% xticks = linspace(1, length(latency_cue), numel(xticklabels));
...
% 
% savefig('w:\WANDER\images\article\ERF_dipole');
%     print('-deps','w:\WANDER\images\article\ERF_dipole.eps']);
% 
% %% FFT
% 
...
% [~, ihigh_probe] = find(f_probe>16,1,'first');
%         
% % plot CUE FFT
% figure;
% subplot(2,1,1);
