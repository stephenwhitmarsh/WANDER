function [FFT] = WANDER_FFT_fourier(isubject,force,rootpath)
% based on WANDER_TFR_phaselocking_balanced_V2b 
disp(['Working on Subjectnr. ' num2str(isubject)]);

if rootpath == 1
    fname_FFT_fourier = ['w:\WANDER\data\PFR\s' num2str(isubject) '_FFT_fourier.mat'];
else
    fname_FFT_fourier = ['/shared/projects/project_wander/WANDER/data/PFR/s' num2str(isubject) '_FFT_fourier.mat'];
end

if exist(fname_FFT_fourier,'file') && force ~= 1
    fprintf('Returning FFT_fourier \n');
